    ...
    render(){
        return(
            <Container>
                ...
                {(this.props.selectedValue == undefined) && <Redirect to="/" /> }
                ...
                <Container text>
                    ...
                    <p style={{padding:'30px'}}>
                        결제정보<br /><br />
                    모임 일자: {this.props.meetingDateInfo} <br />
                입금 계좌: {this.getAccount(this.props.selectedValue)}<br />
                ...
                4. 환불/취소/변경은 {this.getDeadLine(this.props.selectedValue)}전 까지 가능하며,
                링쿠 카카오톡 플러스 친구(@linku)로 신청하셨던 전화번호와 환급받으실 계좌번호를 보내주시면 환급됩니다.<br/>
                    </p>
                </Container>
...
const mapStateToProps = (state) => {
    return {
        selectedValue: state.meetingCardInfos.selectedValue,
        meetingDateInfo: state.meetingCardInfos.meetingDateInfo,
        isCurrent: state.meetingCardInfos.isCurrent
    };
};
export default withRouter(connect(mapStateToProps)(PaymentApplyContents));
