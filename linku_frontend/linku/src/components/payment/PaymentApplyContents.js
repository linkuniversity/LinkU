...
const PaymentApplyContents = ({history, location}) => (
    <Container>
        ...
        <Container text>
            ...
            <p style={{padding:'30px'}}>
                결제정보<br /><br />
                모임 일자: {new URLSearchParams(location.search).get('paymentInfo')} <br />
            입금 계좌: {getAccount(parseInt(new URLSearchParams(location.search).get('selectedValue')), new URLSearchParams(location.search).get('isCurrent'), new URLSearchParams(location.search).get('isPrearranged'))}<br />
        ...
        4. 환불/취소/변경은 {getDeadLine(parseInt(new URLSearchParams(location.search).get('selectedValue')), new URLSearchParams(location.search).get('isCurrent'), new URLSearchParams(location.search).get('isPrearranged'))}전 까지 가능하며, 링쿠 카카오톡 플러스 친구(@linku)로 신청하셨던 전화번호와 환급받으실 계좌번호를 보내주시면 환급됩니다.<br/>
            </p>
        </Container>
